		...
		if msgType = "roSGNodeEvent"
			...
			if userRequest.requestId <> invalid
				...
				if responseStr <> invalid and responseStr <> ""
					...
					else if GetInterface(userRequest.filter, "ifArray") <> invalid AND userRequest.filter.Count() > 0

						responseFiltered = {}
						for i=0 to userRequest.filter.Count() - 1

							field = userRequest.filter[i]
							if responseProcessed[field] <> invalid

								responseFiltered[field] = responseProcessed[field]
							end if
						end for
						responseProcessed = responseFiltered
					end if
				end if
